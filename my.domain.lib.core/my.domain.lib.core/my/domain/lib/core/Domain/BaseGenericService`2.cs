...
// Type: my.domain.lib.core.Domain.BaseGenericService`2
...
{
    public abstract class BaseGenericService<TEntity, TId> : BaseGenericReadOnlyService<TEntity, TId>
    where TEntity : EntityTypedId<TId>
    {
        public IGenericRepository Repository { get; }

        protected BaseGenericService(IGenericRepository repository)
        ...
        public virtual void Remove(TId id)
        {
            ...
            if (e.Cancel)
                return;
            this.Repository.Remove<TEntity, TId>(id);
